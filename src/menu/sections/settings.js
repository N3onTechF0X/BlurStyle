...
export const createSettingsSection = (container) => {
  const settingsCard = createCard('Manage settings');
  const exportButton = createButton('Export settings', () => {
    try {
      const settings = {};
      ...
      const exportLink = document.createElement('a'); exportLink.setAttribute('href', dataUri); exportLink.setAttribute('download', 'blurstyle-settings.json'); exportLink.click(); showNotification('Settings exported successfully', 'success');}
        catch (error) {console.error('Ошибка при экспорте настроек:', error); showNotification('Error with export settings', 'error');}}, 'bs-btn-primary');
  ...
  const importButton = createButton('Import settings', () => {importInput.click();});
            ...
            reader.onload = (e) => {
              try {
                const settings = JSON.parse(e.target.result);
                  Object.entries(settings).forEach(([key, value]) => {localStorage.setItem(key, value);}); showNotification('The settings have been successfully imported, to apply them, reload the page', 'success');}
                    catch (error) {console.error('Ошибка при импорте:', error); showNotification('Error importing settings: ' + error.message, 'error');}}; reader.readAsText(file);}}); importContainer.appendChild(importButton); importContainer.appendChild(importInput);
  ...
  const resetButton = createButton('Reset settings', () => {
    ...
    const modalTitle = document.createElement('h3'); modalTitle.textContent = 'Reset settings?'; modalTitle.className = 'bs-modal-title';
    const modalText = document.createElement('p'); modalText.textContent = 'Are you sure you want to reset all settings to default?'; modalText.style.margin = '0 0 24px 0'; modalText.style.color = 'rgba(255, 255, 255, 0.7)'; modalText.style.textAlign = 'center';
    ...
    const cancelBtn = document.createElement('button'); cancelBtn.textContent = 'Cancel'; cancelBtn.className = 'bs-btn'; cancelBtn.style.background = 'rgba(255, 255, 255, 0.1)';
    const confirmBtn = document.createElement('button'); confirmBtn.textContent = 'Reset'; confirmBtn.className = 'bs-btn'; confirmBtn.style.background = 'rgba(255, 107, 107, 0.3)'; buttonContainer.appendChild(cancelBtn); buttonContainer.appendChild(confirmBtn);
          ...
          prefixes.forEach(prefix => {
            const keys = getAllKeysStartingWith(prefix);
              keys.forEach(key => removeFromLocalStorage(key));});
                specificKeys.forEach(key => removeFromLocalStorage(key)); closeModalWithAnimation(modal); showNotification('The settings have been successfully reset, to apply them, reload the page', 'error');});}, 'bs-btn-danger');
